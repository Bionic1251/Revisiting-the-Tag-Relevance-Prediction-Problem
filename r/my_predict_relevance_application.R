...
trainPrecictTest <- function(trainDataPath, testDataPath, savePredictionsPath, testMode=FALSE, evalMode=FALSE, samplingRatio=.05, smallData=FALSE) {
	library(lme4)
	source("models.R")
	source("dataset.R")
	source("tagfit.R")
  ...
  #trainDataPath="libdat/temp/train0.csv"
  #testDataPath = "libdat/temp/test0.csv"
  #savePredictionsPath = "libdat/temp/predictions_r0.txt"
  # print(trainDataPath)
  # print(testDataPath)
...
predictTagRelevance <- function(trainDataFile, predictDataFile, predictionsFile, testMode=FALSE, evalMode=FALSE, samplingRatio=.05, smallData=FALSE) {
  ...
  #dataSurvey <- read.table(file="movielens-tagnav/r/out/dataSurvey.csv", header=TRUE, sep=",")
