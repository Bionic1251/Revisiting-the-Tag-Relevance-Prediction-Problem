#!/usr/bin/env python
"""
Code from Tag revisiting train_pytorch.py
...
import pandas as pd
from src.models.train_pytorch_settings import *
...
#ENCODINGS = {WORD2VEC_ENCODING: tags_word2vec, ONE_HOT_ENCODING: tags_one_hot}
ENCODINGS = {ONE_HOT_ENCODING: tags_one_hot}

ENCODING = WORD2VEC_ENCODING
ENCODING = ONE_HOT_ENCODING

DIMENSION_ONE_HOT = len(tags_one_hot[list(tags_one_hot.keys())[0]]) # 1071
...
def create_train_validation_dataloader(df):
    ...
    train_loader = torch.utils.data.DataLoader(TagnavDataset(df), batch_size=BATCH_SIZE, shuffle=False, sampler=train_sampler)
    validation_loader = torch.utils.data.DataLoader(TagnavDataset(df), batch_size=BATCH_SIZE, shuffle=False, sampler=validation_sampler)
...
def train(train_data_loader,
          ...
          num_epochs=15,
          model_path_save=get_path('temp/model.bin'),
          perf_json_save_path=get_path('temp/perf.json')):
    ...
    for epoch in range(num_epochs):
        ...
        save_path_epoch_model = get_path(f"temp/model_ep_{epoch}.bin")
...
def calc_mae_for_ten_fold():
    for i in range(10):
        ...
        if mae_r:
            logger.info("MAE {0:1.3} Baseline MAE {1:1.3f} R MAE {2:1.3f}".format(mae, mae_baseline, mae_r))
        else:
            logger.info("MAE {0:1.3} Baseline MAE {1:1.3f} R MAE - not ready".format(mae, mae_baseline))


class TenFoldsData:
    ten_folds_dir = get_path("data/processed/10folds")
...
def run_tenfolds():
    for i in range(10):
        ...
        model_path = get_path(f'temp/model_fold_{i}.bin')
        train(train_loader,
              validation_loader=validation_loader,
              num_epochs=NUM_EPOCHS,
              model_path_save=model_path,
              perf_json_save_path=get_path(f'temp/perf_fold_{str(i)}.json'))

        eval_step(test_loader,
                  model_path_load=model_path,
                  save_predictions_to=get_path(f'data/predictions/predictions_fold_{str(i)}.txt'))
